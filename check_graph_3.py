...
training_data = Cifar10DataFetcher('TRAIN', batch_size=hnet_hparams.batch_size, order='NHWC')
resnet = Resnet(input=training_data.image, hparams=target_hparams, image_params=image_params, weights=None, order='NHWC', graph=graph)
...
training_data = Cifar10DataFetcher('TRAIN', batch_size=hnet_hparams.batch_size, order='NHWC')
image = tf.transpose(training_data.image, [0, 3, 1, 2]) # NHWC to NCHW
resnet = Resnet(input=image, hparams=target_hparams, image_params=image_params, weights=None, order='NCHW', graph=graph)
init = tf.global_variables_initializer()
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    assert len(graph.get_operations()) == 2896 + 2
    assert len(tf.all_variables()) == 101
