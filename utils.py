...
def MultiLayerPerceptron2(input,widths,with_batch_norm=False,where_to_batch_norm=None,train_batch_norm=True,activation_on_last_layer=False,is_training=None,scale=1.0,batchnorm_decay=0.98,activation=tf.nn.relu,name=None,zero_fixer=1e-8):
    """
    Creates a multilayer perceptron (=MLP)
    Args:
        input: input layer to perceptron
    ...
    for i in range(1,len(widths)):
        ...
        if i<(len(widths)-1):
            ...
            if activation_on_last_layer is False:
                ...
                layer_output = activation_on_last_layer(pre_activations, GiveName(name,'layer{:d}_activations'.format(i)))
        if where_to_batch_norm[i]:
            layer_output,params = AddBacthNormalizationOps2(layer_output,is_training,train_batch_norm,batchnorm_decay,zero_fixer,GiveName(name,'layer{:d}_batchnorm'.format(i)))
            batch_norm_params.append(params)
        layer_outputs.append(layer_output)
    layer_outputs = layer_outputs[1:]
    return layer_outputs,layers,batch_norm_params
...
def AddBacthNormalizationOps2(input, is_training, train_BN_params, batchnorm_decay, zero_fixer=1e-8, name=None):
    """
    ...
    def ApplyEmaUpdate():
        with tf.control_dependencies([ema_ops]):
            return tf.identity(batch_means), tf.identity(batch_variances)
    def Average():
        return (ema.average(batch_means), ema.average(batch_variances))
    if isinstance(is_training,bool):
        if is_training:
            means, variances = ApplyEmaUpdate()
        else:
        ...
        means, variances = tf.cond(is_training, ApplyEmaUpdate, Average)
    return tf.nn.batch_normalization(input, means, variances, offsets, scales,zero_fixer,name), (means,variances,offsets,scales)
...
def ConvBN2(x, w, stride, scale, offset, order, batch_type='BATCH_TYPE1', name=None):
    """
    perform convolution and "batch norm", where the batch norm does not calculate means and variances
    Args:
        x:
    ...
    """
    return tf.identity(ConvType1(x, w, stride, order) * scale + offset, name)
