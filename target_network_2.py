...
from utils import ConvBN2,MaxPool
from logging import Logger

class ResnetWeights2():
    """
...
class Resnet2():
    def __init__(self, input, hparams:ResNetHyperParameters, image_params:DataParams, labels=None, train=False, weights:ResnetWeights2=None, order='NCHW', batch_type='BATCH_TYPE1', noise_batch_size=1, graph=None):
        """
    ...
    def __Build(self,input):
        ...
        x = tf.nn.relu(ConvBN2(input, weights.first_layer.w, self.hparams.stride_between_scales, weights.first_layer.bn_scale, weights.first_layer.bn_offset,self.order,self.batch_type))
        ...
        for i in range(num_scales):
            for j in range(self.hparams.num_blocks_per_scale[i]):
                if (i > 0) and (j == 0): # condition checks whether to apply a projection in residual connection
                    x1 = ConvBN2(x, weights.scales[i].blocks[0].projection.w, self.hparams.stride_between_scales, weights.scales[i].blocks[0].projection.bn_scale, weights.scales[i].blocks[0].projection.bn_offset,self.order,self.batch_type)
                else:
                    x1 = x
                for k in range(self.hparams.layers_per_block):
                    if (j == 0) and (k == 0) and (i > 0): # condition checks whether to apply a stride in the convolution
                        ...
                        stride = 1
                    x = ConvBN2(x, weights.scales[i].blocks[j].conv_layers[k].w, stride, weights.scales[i].blocks[j].conv_layers[k].bn_scale, weights.scales[i].blocks[j].conv_layers[k].bn_offset,self.order,self.batch_type)
    ...
    def __CreateWeightsFeedDict(self,weights:ResnetWeights2):
        feed_dict = {}
    ...
    def __CreateWeightVariables(self,batch_size=None):
        ...
        if self.order == 'NCHW':
            ...
            permute = lambda u:u
        weights = ResnetWeights2(self.hparams,self.image_params)
        num_scales = len(self.hparams.num_blocks_per_scale)
